...
/**
 * The indexes the benchmark chart draws the account against.
 *
 * ETFs rather than the indexes themselves, for the same reason the S&P
 * comparison has always used one: the index level is a licensed product and the
 * fund tracking it is free to quote. The percentage return is what the chart
 * shows, and over a year these track their index closely enough that the
 * difference is invisible at this scale.
 *
 * Colours are CSS variable names, resolved at draw time so both themes work.
 */
export const BENCHMARKS = [
  { symbol: 'VOO', name: 'S&P 500', colour: '--blue' },
  { symbol: 'QQQ', name: 'Nasdaq 100', colour: '--amber' },
];

/**
 * Daily closes for one index, cached for the day.
 *
 * Goes through this deployment's own history endpoint, which already accepts
 * any symbol — so comparing against a second index costs no new serverless
 * function, and the Hobby plan's limit of twelve is not worth spending on
 * something already served.
 *
 * The S&P is delegated to `benchmarkSeries` rather than fetched again: it is
 * already cached for the overview's own comparison and for measured betas, and
 * a second copy would mean two requests for one answer.
 */
const indexSeriesCache = new Map();

export async function indexSeries(symbol) {
  if (symbol === BENCHMARK_SYMBOL) return benchmarkSeries();
  if (indexSeriesCache.has(symbol)) return indexSeriesCache.get(symbol);

  const key = `pt_index_${symbol}`;
  try {
    const raw = localStorage.getItem(key);
    const cached = raw ? JSON.parse(raw) : null;
    if (cached?.fetchedAt && Array.isArray(cached.series)
      && Date.now() - cached.fetchedAt < CACHE_TTL_MS) {
      indexSeriesCache.set(symbol, cached.series);
      return cached.series;
    }
  ...
  try {
    const res = await fetch(`/api/history?symbol=${encodeURIComponent(symbol)}&years=2`, {
      credentials: 'same-origin',
    });
    if (!res.ok) return null;
    const json = await res.json();
    const rows = Array.isArray(json?.rows) && json.rows.length >= 2 ? json.rows : null;
    if (!rows) return null;
    indexSeriesCache.set(symbol, rows);
    try {
      localStorage.setItem(key, JSON.stringify({ fetchedAt: Date.now(), series: rows }));
    } catch { /* it simply will not persist */ }
    return rows;
  } catch {
    return null;
  }
...
/** Every benchmark's history, fetched together. Absent ones come back as null. */
export async function benchmarkHistories() {
  const rows = await Promise.all(BENCHMARKS.map((b) => indexSeries(b.symbol).catch(() => null)));
  return BENCHMARKS.map((b, i) => ({ ...b, rows: rows[i] }));
}
