...
/** "16 September", which is how anyone says it out loud. */
...
function renderFedPanel(decision) {
  ...
  if (!decision) { card.style.display = 'none'; return; }
  card.style.display = '';
 ...
 *
 * The month's figures are already on screen by the time this returns, so a slow
 * or missing futures feed costs the page nothing.
 */
async function refreshFedPanel() {
  try {
